...
- **เตือน ESP32 ไม่ออนไลน์ด้วย:** ไม่มีค่าจากเซ็นเซอร์เข้าเกิน **`OFFLINE_ALERT_MIN`** (default 10 นาที, ตั้ง 0 = ปิด) → LINE แจ้งเตือนครั้งเดียวต่อรอบ พอค่ากลับมาเข้า → แจ้ง "กลับมาออนไลน์แล้ว" — ตั้งผ่าน Render Dashboard (มีใน render.yaml `sync: false`) — หมายเหตุ: บน free tier service หลับเมื่อไม่มี request เตือนได้เฉพาะตอน service ตื่น
